...
- GET /api/v1/pokemon?limit=10 → Lists at least 10 Pokémon
...
Calls PokéAPI (detail) and reduces to hp/attack/defense (you choose).
...
The Service fetches attacker/defender + move via repository (PokéAPI), then computes a single attack (opponent does not attack back).
...
2. Battle Simulator
   ...
   - Prompts for a move name (you may show the first 5 moves from your Pokémon stats endpoint or do an extra local fetch via your own API if you implemented /move).
   - Calls POST /api/v1/battle/attack with {attacker, defender, move}.
   - Displays the result (hit/miss, damage, and a short reason).
3. Get Stats for Pokemon
...
Example Terminal Session (what a grader should see)
...
| Architecture & OOP         |  20 | Clear N-tier; Service/Repository classes; clean responsibilities.   |
...
- Simple, deterministic single-attack simulation on /battle/attack.
